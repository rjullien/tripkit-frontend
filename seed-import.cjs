...
async function main() {
  ...
  // 3. Upsert hotels (supports both dict {id: data} and array [{id, ...}] formats)
  let hotelEntries;
  if (Array.isArray(SEED.hotels)) {
    hotelEntries = SEED.hotels.map(h => [h.id, h]);
  } else {
    hotelEntries = Object.entries(SEED.hotels || {});
  }
